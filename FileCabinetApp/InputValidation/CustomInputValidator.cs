...
namespace FileCabinetApp.InputValidation
{
    ...
    public class CustomInputValidator : IInputValidator
    {
        ...
        public Tuple<bool, string, DateTime> DateConverter(string dateInput)
        {
            ...
            if (correctDateFormat && (dateOfBirth < new DateTime(1950, 1, 1) || dateOfBirth > DateTime.Today))
            {
                return new Tuple<bool, string, DateTime>(
                    false,
                    "The date of birth cannot be earlier than 1-Jan-1950 or later than the current date.",
                    dateOfBirth);
            }
        ...
        public Tuple<bool, string> NameValidator(string nameInput)
        {
            ...
            if (nameInput.Length < 2 || nameInput.Length > 30)
            {
                return new Tuple<bool, string>(false, "First or last name cannot be shorter than 2 characters and longer than 30 characters.");
            }
        ...
        {
            if (genderInput != GenderFemale && genderInput != GenderMale)
            {
                return new Tuple<bool, string>(false, "Gender can only be F or M.");
            }
